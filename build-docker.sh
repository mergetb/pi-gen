...
if [[ "${binfmt_misc_required}" == "1" ]]; then
  if ! qemu_aarch64=$(which qemu-aarch64-static) ; then
  ...
  if ! grep -q "^interpreter ${qemu_aarch64}" /proc/sys/fs/binfmt_misc/qemu-aarch64* ; then
    # Register qemu-aarch64 for binfmt_misc
    reg="echo ':qemu-aarch64-rpi:M::"\
"\x7fELF\x02\x01\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x00\xb7\x00:"\
"\xff\xff\xff\xff\xff\xff\xff\x00\xff\xff\xff\xff\xff\xff\xff\xff\xfe\xff\xff\xff:"\
"${qemu_aarch64}:F' > /proc/sys/fs/binfmt_misc/register"
